...
@testset "PORTA.read_poi()" begin
    @testset "error thrown if file not `.poi`" begin
        @test_throws DomainError PORTA.PORTA.read_poi("./test/files/imaginary_file.txt")
    end

    @testset "read test/files/example1.poi" begin
        poi = PORTA.PORTA.read_poi("./test/files/example1.poi")
    ...
    @testset "read test/files/example2.poi" begin
        poi = PORTA.PORTA.read_poi("./test/files/example2.poi")
        @test poi isa POI{Rational{Int}}
...
@testset "PORTA.read_ieq()" begin
    ...
    @testset "read test/files/example1.ieq" begin
        ieq = PORTA.PORTA.read_ieq("./test/files/example1.ieq")
...
@testset "PORTA.write_poi()" begin
    ...
    @testset "filename may end in `.poi`" begin
        filepath = PORTA.write_poi("filename_test.poi", POI(vertices=[1 0 0;0 -2 0;0 0 1]), dir=test_dir)
        @test filepath == test_dir * "/filename_test.poi"
    end

    @testset "simple vertices" begin
        @test !isfile("./test/files/porta_tmp/int_vert_test.poi")
        filepath = PORTA.write_poi("int_vert_test", POI(vertices=[1 0 0;0 -2 0;0 0 1]), dir=test_dir)
    ...
    @testset "simple rays" begin
        ...
        filepath = PORTA.write_poi("int_ray_test", POI(rays=[1 2 3;4 5 6;7 8 9], valid=[3 3 3]), dir=test_dir)
    ...
    @testset "rational vertices" begin
        filepath = PORTA.write_poi("rational_vert_test", POI(vertices=[1//2 3//4;5//6 7//8;0 -1]), dir=test_dir)
...
@testset " PORTA.write_ieq()" begin
    ...
    @testset "simple inequalities" begin
        ...
        filepath = PORTA.write_ieq("int_ineq_test", IEQ(inequalities=[1 0 1;0 -2 -2;1 -1 1]), dir=test_dir)
    ...
    @testset "simple rational equalities" begin
        ...
        filepath = PORTA.write_ieq("rational_eq_test", IEQ(equalities=[1//2 3//4 -5 4;0 -2//3 -2 0]), dir=test_dir)
